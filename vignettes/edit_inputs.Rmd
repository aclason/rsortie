...
This vignette will describe how to create one or more new SORTIE parameter files that contain updated information based on user-provided values or chunks of xml code
...
To set up a base parameter file and run SORTIE, you must also download and install SORTIE - instructions are on the main page.

rsortie interacts with the SORTIE parameter files. These are .xmls that contain the list of parameters within behaviours defined in the SORTIE GUI. rsortie cannot interact with or change elements that do not exist already in the base xml file, so it is important to understand the GUI to select all the behaviours required to run a SORTIE simulation. Importantly, if you intend to primarily use rsortie to assign values to parameters within the selected behaviours, you must assign a non-zero value to those parameters, or those lines do not get printed to the base xml.
